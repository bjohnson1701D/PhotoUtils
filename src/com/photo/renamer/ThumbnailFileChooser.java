...
public class ThumbnailFileChooser extends JFileChooser {
    ...
    private final Pattern imageFilePattern = Pattern.compile(".+?\\.(png|jpe?g|mp?g|mp4|mov|gif|tiff?)$", Pattern.CASE_INSENSITIVE);
    ...
    public ThumbnailFileChooser(String name) {
        super(name);
    } 
    ...
    private class ThumbnailView extends FileView {
        ...
        public Icon getIcon(File file) {
            if (!imageFilePattern.matcher(file.getName()).matches()) {
            	...
            	Image img = ((ImageIcon)i).getImage().getScaledInstance(ICON_SIZE, ICON_SIZE, Image.SCALE_SMOOTH);
